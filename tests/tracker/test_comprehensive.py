...
class TestNormalTalking:
    ...
    def test_first_paragraph_word_by_word(self) -> None:
        ...
        for i, word in enumerate(words):
            ...
            pos: ScriptPosition = tracker.update(transcript)
    ...
    def test_multi_paragraph_tracking(self) -> None:
        ...
        intro_word_count: int = len(intro.split())
        ...
        section_start: str = intro + \
            " how it works the system listens to your voice through the microphone"
        pos = tracker.update(section_start)

        assert pos.word_index > intro_word_count, "Should have advanced into 'How It Works' section"

    def test_section_by_section_tracking(self) -> None:
        ...
        first_section_pos: int = tracker.optimistic_position
        ...
        second_pos: int = tracker.optimistic_position
        ...
        third_pos: int = tracker.optimistic_position
        assert third_pos > second_pos

    def test_continuous_accurate_speech_through_features_section(self) -> None:
    ...
    def test_position_accuracy_within_tolerance(self) -> None:
...
class TestMinorSpeechFaults:
    ...
    def test_word_order_swap(self) -> None:
        ...
        speech: str = "welcome autocue to hello and welcome to this demonstration"
        pos: ScriptPosition = tracker.update(speech)
    ...
    def test_dropped_word(self) -> None:
        ...
        speech: str = "welcome to autocue and welcome to this demonstration"
        pos: ScriptPosition = tracker.update(speech)
    ...
    def test_multiple_minor_faults_across_paragraphs(self) -> None:
        """Multiple minor faults through script should maintain reasonable tracking."""
        tracker: ScriptTracker = ScriptTracker(SAMPLE_SCRIPT)

        # First part with some fillers and a repeat
        para1: str = "welcome to autocue um hello and welcome to this this demonstration"
        tracker.update(para1)
        pos1: int = tracker.optimistic_position

        # Continue with slight variations
        para2: str = para1 + " of the uh autocue system today im gonna walk you through"
        ...
        para3: str = para2 + " how this um teleprompter works"
        tracker.update(para3)
        pos3: int = tracker.optimistic_position

        assert pos3 > pos2, "Should continue advancing"

    def test_tracking_accuracy_with_faults_within_three_words(self) -> None:
    ...
    def test_gonna_instead_of_going_to(self) -> None:
...
class TestBacktracking:
    ...
    def test_backtrack_to_beginning_of_paragraph(self) -> None:
        ...
        position_before = tracker.current_word_index
        assert position_before > 10, "Should have advanced significantly"
        ...
        assert tracker.optimistic_position < position_before, "Should have moved back"

    def test_backtrack_to_earlier_section(self) -> None:
        ...
        tracker.update(speech)
        original_pos: int = tracker.optimistic_position
    ...
    def test_backtrack_mid_sentence(self) -> None:
        ...
        full: str = preamble + " the system listens to your voice through the microphone and uses"
        tracker.update(full)
        mid_pos: int = tracker.optimistic_position
    ...
    def test_words_after_backtrack_track_from_new_position(self) -> None:
...
class TestForwardJumping:
    ...
    def test_skip_sentence_detected(self) -> None:
    ...
    def test_skip_bullet_point(self) -> None:
        ...
        tracker.update(intro)
        pos_after_first_bullet: int = tracker.optimistic_position

        # Skip "backtrack detection" bullet, go to "low latency"
        skip_speech: str = intro + " low latency because everything runs locally"
    ...
    def test_forward_jump_moves_forward(self) -> None:
        """Forward jumping should update the current position."""
        tracker: ScriptTracker = ScriptTracker(SAMPLE_SCRIPT)

        # Start at beginning with correct words
        tracker.update(
            "welcome to autocue hello and welcome to this demonstration")
        position_before: int = tracker.current_word_index
        ...
        # Current position should have increased
        assert tracker.current_word_index >= position_before
...
class TestFalsePositiveBacktrackPrevention:
    ...
    def test_repeated_phrase_no_false_backtrack(self) -> None:
        """Repeated phrases later in script shouldn't trigger false backtrack."""
        # Script with repeated phrase "the system"
        script_with_repeats: str = """The system is great. It works well.

Later on, the system handles more tasks. The system never fails."""

        tracker: ScriptTracker = ScriptTracker(script_with_repeats)
        ...
        tracker.update(
            "the system is great it works well later on the system handles")
    ...
    def test_common_words_no_false_backtrack(self) -> None:
        ...
        position_in_section_2: int = tracker.optimistic_position
    ...
    def test_similar_sentence_structure_no_backtrack(self) -> None:
        ...
        # Continue speaking
        speech2: str = speech1 + " im going to walk you through"
        tracker.update(speech2)
        pos2: int = tracker.optimistic_position

        assert pos2 > pos1, "Should continue forward"

    def test_optimistic_position_trusted_with_small_deviation(self) -> None:
    ...
    def test_confident_current_position_prevents_backtrack(self) -> None:
...
class TestNearestBacktrack:
    ...
    def test_backtrack_to_nearest_repeated_phrase(self) -> None:
        ...
        script: str = """Start here. The quick brown fox jumps.
...
Later section. The quick brown fox jumps one more time. End."""
        ...
        tracker: ScriptTracker = ScriptTracker(script, max_jump_distance=50)
        ...
        pos_at_later: int = tracker.optimistic_position
    ...
    def test_max_jump_distance_prevents_distant_match(self) -> None:
        """max_jump_distance should prevent matching very distant similar text."""
        tracker: ScriptTracker = ScriptTracker(
            SAMPLE_SCRIPT, max_jump_distance=20)
    ...
    def test_nearby_similar_sentences_prefer_nearest(self) -> None:
        """When multiple similar sentences exist, prefer the one nearest current position."""
        script: str = """First: speak clearly and naturally.
...
class TestFalsePositiveForwardJumpPrevention:
    ...
    def test_similar_future_sentence_no_false_jump(self) -> None:
        """Similarity to a future sentence shouldn't cause false forward jump."""
        # Script where similar phrases appear later
        script: str = """Start with the basics. Learn the fundamentals first.
...
Finally, master the basics. You have completed training."""
        ...
        pos_after_first: int = tracker.optimistic_position

        # Continue with "now" - should NOT jump to "finally master the basics"
        tracker.update(
            "start with the basics learn the fundamentals first now practice")
    ...
    def test_repeated_later_content_no_jump(self) -> None:
    ...
    def test_optimistic_tracking_prevents_forward_jump(self) -> None:
    ...
    def test_forward_jump_only_when_genuinely_ahead(self) -> None:
...
class TestEdgeCasesWithSampleScript:
    ...
    def test_script_beginning_tracking(self) -> None:
        ...
        pos: ScriptPosition = tracker.update("welcome")
        assert pos.word_index == 1

    def test_script_end_tracking(self) -> None:
        """Tracking should handle reaching the end of script."""
        # Use a shorter script for this test
        short_script: str = "Hello world end."
        tracker: ScriptTracker = ScriptTracker(short_script)
    ...
    def test_empty_update(self) -> None:
        ...
        pos: int = tracker.optimistic_position
...
class TestProgressTracking:
    ...
    def test_progress_increases_with_speech(self) -> None:
        """Progress should increase as we speak through the script."""
        tracker: ScriptTracker = ScriptTracker(SAMPLE_SCRIPT)

        # Use correct starting words
        tracker.update("welcome to autocue hello and welcome to this")
        progress1: float = tracker.progress
        assert progress1 > 0
        ...
        tracker.update(speech2)
        progress2: float = tracker.progress
    ...
    def test_progress_calculation_accuracy(self) -> None:
        ...
        short_script: str = "one two three four five six seven eight nine ten"
        tracker: ScriptTracker = ScriptTracker(short_script)
...
class TestIntegrationScenarios:
    ...
    def test_complete_paragraph_then_backtrack_and_continue(self) -> None:
        ...
        tracker.update(
            "today im going to walk you through how this teleprompter")
        tracker.update(
            "today im going to walk you through how this teleprompter works")
        tracker.update(
            "today im going to walk you through how this teleprompter works and")
        final_pos: int = tracker.optimistic_position
    ...
    def test_natural_speaking_with_pauses_and_fillers(self) -> None:
    ...
    def test_reading_full_section(self) -> None:
        ...
        tracker.update(intro)
        pos_before_section: int = tracker.optimistic_position

        # Read the section
        tracker.update(intro + " " + how_it_works)
        pos_after_section: int = tracker.optimistic_position
